...
#		s = ser.readline().decode() #old way of reading the test
		s = ser.read(4)


		try:
			arduino_time=int.from_bytes(s, "little") #the arduino's own time, in microseconds.  Strip the newlines
		except ValueError as e:
			print("error ",e," value of s: ",s.strip()) # might be problematic
...
#		outfile.write(str(arduino_time)+"\t"+str(actual_time)+"\t"+str(time_elapsed))
		...
		if args.rate: #print the rate information
			if count<integration_count:
				...
				outfile.write("\t" +str(int(60*integration_count/1e-6/float(times[-1]-times[0])))+ "\n")
		else:
			outfile.write("\t -1 \n")
